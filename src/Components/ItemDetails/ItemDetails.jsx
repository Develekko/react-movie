        ...
        <div className="container pt-5">
          ...
          {similar.length > 0 ? <div className="my-5 similar">
            ...
            <div className='container text-center'>
              ...
              >
                {similar.filter(img => img.poster_path !== null).map((movie, index) => <SwiperSlide onClick={() => window.scrollTo(0, 0)} key={index}><Link to={`/movie-details/${movie.id}/${media_type}`}><img className='img-fluid' src={imgPath + movie.poster_path} title={movie.name} alt={movie.name} /></Link></SwiperSlide>)}
              </Swiper></> : null}
            </div>
            <hr />
          </div> : null}
